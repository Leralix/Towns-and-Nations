...
public class AddVassalMenu extends IteratorGUI {
    ...
    public AddVassalMenu(Player player, Territory territoryData){
    ...
    private List<GuiItem> potentialVassals() {
        ...
        for (Territory potentialVassal : territoryData.getPotentialVassals()) {
            if (territoryData.getVassalsID().contains(potentialVassal.getID()) || potentialVassal.containsVassalisationProposal(territoryData))
                continue;

            guiItems.add(potentialVassal.getIconWithInformationAndRelation(territoryData, tanPlayer.getLang())
                    .setClickToAcceptMessage((territoryData instanceof Nation) ? Lang.GUI_NATION_INVITE_REGION_DESC1 : Lang.GUI_REGION_INVITE_TOWN_DESC1)
                    .setAction(action -> {
