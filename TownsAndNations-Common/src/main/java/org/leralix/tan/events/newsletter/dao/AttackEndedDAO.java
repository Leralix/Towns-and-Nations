...
import org.leralix.tan.events.newsletter.news.AttackEndedNewsletter;
...
public class AttackEndedDAO extends NewsletterSubDAO<AttackEndedNewsletter> {

    private static final String TABLE_NAME = "attack_ended_newsletter";

    public AttackEndedDAO(DataSource dataSource) {
        super(dataSource);
    }
    ...
    @Override
    public void save(AttackEndedNewsletter newsletter, Connection conn) {
    ...
    @Override
    public AttackEndedNewsletter load(UUID id, long date, Connection conn) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    return new AttackEndedNewsletter(id, date, attackingTerritoryID, defendingTerritoryID);
                }
            }
        } catch (SQLException e) {
