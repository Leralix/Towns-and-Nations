...
public class TributePaidTaxLine extends ProfitLine {

    public TributePaidTaxLine(Territory territory) {
        super(territory);
    }
    ...
    public double getMoney() {
        double amount = 0;
        for (Tribute tribute : TownsAndNations.getPlugin().getTributeStorage().getTributeOfTributary(territoryData)) {
    ...
    public List<FilledLang> getLine() {
        ...
        for (Tribute tribute : TownsAndNations.getPlugin().getTributeStorage().getTributeOfTributary(territoryData)) {
            Territory territory = TerritoryUtil.getTerritory(tribute.getMasterID());
            if (territory == null) {
                continue;
            }
            res.add(Lang.TRIBUTE_PAID_TAX_LINE.get(
