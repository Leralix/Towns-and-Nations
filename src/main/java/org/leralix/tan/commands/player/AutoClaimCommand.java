    ...
    public void perform(Player player, String[] args){
        ...
        switch (message) {
            case "town" -> {
                ...
                TanChatUtils.message(player, Lang.AUTO_CLAIM_ON_FOR.get(langType, ChunkType.TOWN.getName()));
            }
            case "region" -> {
                PlayerAutoClaimStorage.addPlayer(player, ChunkType.REGION);
                TanChatUtils.message(player, Lang.AUTO_CLAIM_ON_FOR.get(langType, ChunkType.REGION.getName()));
            }
            case "stop" -> {
