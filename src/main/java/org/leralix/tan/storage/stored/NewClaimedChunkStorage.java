...
public class NewClaimedChunkStorage extends DatabaseStorage<ClaimedChunk2>{
    ...
    /**
     * Get a chunk synchronously from cache (FAST - no DB blocking)
     * This method checks the cache first and returns immediately.
     * If not in cache, returns null and triggers async load in background.
     *
     * IMPORTANT: This method is designed for event listeners that cannot block.
    ...
    /**
     * @deprecated This method blocks the calling thread - avoid using in event listeners!
     * Use getFromCacheOrNull() instead for non-blocking access.
     */
    @Deprecated
    public ClaimedChunk2 getSync(String id) {
        try {
            ...
            TownsAndNations.getPlugin().getLogger().warning(
                "Error getting claimed chunk data synchronously: " + e.getMessage()
            );
            return null;
        }
    }
    ...
    protected void createIndexes() {
        ...
        String createOwnerIndexSQL = "CREATE INDEX IF NOT EXISTS idx_chunk_owner ON " + TABLE_NAME +
            " ((CAST(JSON_EXTRACT(data, '$.ownerID') AS CHAR(255))))";

        try (Connection conn = getDatabase().getDataSource().getConnection();
             Statement stmt = conn.createStatement()) {
            stmt.execute(createOwnerIndexSQL);
            TownsAndNations.getPlugin().getLogger().info("Created index idx_chunk_owner on " + TABLE_NAME);
        } catch (SQLException e) {
            // If functional index fails (older MySQL versions), try regular column
            TownsAndNations.getPlugin().getLogger().warning(
                "Could not create functional index on " + TABLE_NAME + ", this is normal for older MySQL versions: " + e.getMessage()
            );
        }
    }
    ...
    /**
     * Check if all adjacent chunks are claimed by the same territory.
     * This method uses batch loading to avoid N+1 queries.
     * @param chunk The center chunk
     * @param territoryID The territory ID to check
     * @return CompletableFuture with result
     */
    public CompletableFuture<Boolean> isAllAdjacentChunksClaimedBySameTerritoryAsync(Chunk chunk, String territoryID) {
        ...
        List<CompletableFuture<ClaimedChunk2>> futures = adjacentChunkKeys.stream()
                .map(this::get)
                .toList();
    ...
    /**
     * @deprecated This method blocks the thread - use isAllAdjacentChunksClaimedBySameTerritoryAsync() instead
     */
    @Deprecated
    public boolean isAllAdjacentChunksClaimedBySameTerritory(Chunk chunk, String territoryID) {
        try {
            ...
            TownsAndNations.getPlugin().getLogger().warning(
                "Error checking adjacent chunks: " + e.getMessage()
            );
            return false;
        }
    }
    ...
    /**
     * Check if at least one adjacent chunk is claimed by the same territory.
     * This method uses batch loading to avoid N+1 queries.
     * @param chunk The center chunk
     * @param townID The territory ID to check
     * @return CompletableFuture with result
     */
    public CompletableFuture<Boolean> isOneAdjacentChunkClaimedBySameTerritoryAsync(Chunk chunk, String townID) {
        List<String> adjacentChunkKeys = Arrays.asList(
                getChunkKey(chunk.getX() + 1, chunk.getZ(), chunk.getWorld().getUID().toString()),
                getChunkKey(chunk.getX() - 1, chunk.getZ(), chunk.getWorld().getUID().toString()),
                getChunkKey(chunk.getX(), chunk.getZ() + 1, chunk.getWorld().getUID().toString()),
    ...
    /**
     * @deprecated This method blocks the thread - use isOneAdjacentChunkClaimedBySameTerritoryAsync() instead
     */
    @Deprecated
    ...
    /**
     * Preload chunks in a region into cache (async, non-blocking)
     * Call this when loading a world region to populate cache
     ...
     */
    public CompletableFuture<Void> preloadChunksAsync(int centerX, int centerZ, String worldID, int radius) {
