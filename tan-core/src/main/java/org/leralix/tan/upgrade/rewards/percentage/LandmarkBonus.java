...
public class LandmarkBonus extends IndividualStat implements AggregatableStat<LandmarkBonus> {
  ...
  public FilledLang getStatReward(LangType langType, int level, int maxLevel) {
    ...
    if (level >= maxLevel) {
      return Lang.UPGRADE_LINE_PERCENT_MAX.get(
          Lang.LANDMARK_BONUS.get(langType), Integer.toString(percentValue));
    } else {
      return Lang.UPGRADE_LINE_PERCENT.get(
          Lang.LANDMARK_BONUS.get(langType),
          Integer.toString(percentValue),
          Integer.toString(percentValueNext));
    }
  }
  ...
  public FilledLang getStatReward(LangType langType) {
    ...
    return Lang.UPGRADE_LINE_PERCENT_MAX.get(
        Lang.LANDMARK_BONUS.get(langType), Double.toString(percentValue));
  }
