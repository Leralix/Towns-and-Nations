...
public class WarTimeSlot {
    ...
    try {
      ...
      TownsAndNations.getPlugin()
          ...
          .warning(
              "The time slot \""
                  + value
                  + "\" contains an invalid time format. It will be ignored.");
      return null;
    }
  }
  ...
  public List<FilledLang> getPrintedTimeSlots() {
    ...
    for (Range range : timeSlots) {
      ...
      printedTime.add(
          Lang.AUTHORIZED_ATTACK_TIME_SLOT_SINGLE.get(
  ...
  public boolean canWarBeDeclared(Instant dateTime) {
    LocalDateTime localDateTime =
        LocalDateTime.ofInstant(
            dateTime, TimeZoneManager.getInstance().getTimezoneEnum().toZoneOffset());
