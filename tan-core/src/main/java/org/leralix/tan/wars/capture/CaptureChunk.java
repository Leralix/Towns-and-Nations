...
public class CaptureChunk {
  ...
  public CaptureChunk(
      int initialScore, TerritoryChunk territoryChunk, TerritoryData mainAttacker, String id) {
  ...
  private Optional<Fort> getFortProtecting() {
    for (Fort fort : territoryChunk.getOccupier().getAllControlledFort()) {
      if (fort.getPosition().getDistance(territoryChunk.getMiddleVector2D())
          <= Constants.getFortProtectionRadius()) {
  ...
  private String generateMessage(Optional<Fort> fortProtectingChunk) {
    ...
    if (fortProtectingChunk.isPresent()) {
      message =
          Lang.WAR_INFO_CHUNK_PROTECTED.get(
              Lang.getServerLang(),
      ...
      message =
          Lang.WAR_INFO_CONTESTED.get(
              Lang.getServerLang(), Integer.toString(score), nbAttackers, nbDefenders);
    } else if (isCaptured()) {
      message =
          Lang.WAR_INFO_CHUNK_CAPTURED.get(
              Lang.getServerLang(),
