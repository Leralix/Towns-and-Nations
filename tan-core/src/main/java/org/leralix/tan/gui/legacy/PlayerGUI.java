...
public class PlayerGUI {
  ...
  public static void openOwnedLandmark(Player player, TownData townData, int page) {
    ...
    Gui gui =
        GuiUtil.createChestGui(
            Lang.HEADER_TOWN_OWNED_LANDMARK.get(tanPlayer, String.valueOf(page + 1)), 6);
    ...
    for (Landmark landmark : LandmarkStorage.getInstance().getLandmarkOf(townData)) {
      ...
      GuiItem landmarkButton =
          ItemBuilder.from(landmark.getIcon(tanPlayer.getLang()))
  ...
  public static void openChunkGeneralSettings(Player player, TerritoryData territoryData) {
    ...
    gui.setDefaultClickAction(event -> event.setCancelled(true));
    Map<GeneralChunkSetting, Boolean> generalSettings =
        territoryData.getChunkSettings().getChunkSetting();

    for (GeneralChunkSetting generalChunkSetting : GeneralChunkSetting.values()) {

      GuiItem guiItem =
          ItemBuilder.from(
                  generalChunkSetting.getIcon(
                      generalSettings.getOrDefault(generalChunkSetting, false),
                      tanPlayer.getLang()))
              .asGuiItem(
                  event -> {
                    event.setCancelled(true);
                    if (!territoryData.doesPlayerHavePermission(
                        player, RolePermission.MANAGE_CLAIM_SETTINGS)) {
                      TanChatUtils.message(
                          player, Lang.PLAYER_NO_PERMISSION.get(tanPlayer), NOT_ALLOWED);
                      return;
                    }
                    generalSettings.put(
                        generalChunkSetting, !generalSettings.get(generalChunkSetting));
  ...
  public static void openTownChunkMobSettings(Player player, int page) {
    ...
    Gui gui =
        GuiUtil.createChestGui(
            Lang.HEADER_MOB_SETTINGS.get(tanPlayer, Integer.toString(page + 1)), 6);

    TownData townData = TownDataStorage.getInstance().getSync(player);
    ClaimedChunkSettings chunkSettings = townData.getChunkSettings();

    ArrayList<GuiItem> guiLists = new ArrayList<>();
    Collection<MobChunkSpawnEnum> mobCollection =
        MobChunkSpawnStorage.getMobSpawnStorage().values();
    ...
    for (MobChunkSpawnEnum mobEnum : mobCollection) {
      ...
      if (upgradeStatus.isUnlocked()) {
        ...
        status.add(
            Lang.GUI_TOWN_CHUNK_MOB_SETTINGS_STATUS_LOCKED2.get(tanPlayer, Integer.toString(cost)));
      }
          ...
          new GuiItem(
              ...
              event -> {
                ...
                if (!townData.doesPlayerHavePermission(
                    tanPlayer, RolePermission.MANAGE_MOB_SPAWN)) {
                  ...
                  if (townData.getBalance() < cost) {
                    TanChatUtils.message(
                        ...
                        Lang.TERRITORY_NOT_ENOUGH_MONEY.get(
                            ...
                            Double.toString(cost - townData.getBalance())));
                    return;
                  }
  ...
  public static void openPlayerListForChunkPermission(
      Player player, TerritoryData territoryData, ChunkPermissionType type, int page) {
    ITanPlayer tanPlayer = PlayerDataStorage.getInstance().getSync(player);
  ...
  public static void openAddPlayerForChunkPermission(
      Player player, TerritoryData territoryData, ChunkPermissionType type, int page) {
    ITanPlayer tanPlayer = PlayerDataStorage.getInstance().getSync(player);
  ...
  public static void openAddVassal(Player player, TerritoryData territoryData, int page) {
    ITanPlayer tanPlayer = PlayerDataStorage.getInstance().getSync(player);
    Gui gui =
        GuiUtil.createChestGui(Lang.HEADER_VASSALS.get(tanPlayer, Integer.toString(page + 1)), 6);

    List<GuiItem> guiItems = new ArrayList<>();

    for (TerritoryData potentialVassal : territoryData.getPotentialVassals()) {
      if (territoryData.isVassal(potentialVassal)
          || potentialVassal.containsVassalisationProposal(territoryData)) continue;

      ItemStack territoryIcon =
          potentialVassal.getIconWithInformationAndRelation(territoryData, tanPlayer.getLang());
      HeadUtils.addLore(territoryIcon, Lang.GUI_REGION_INVITE_TOWN_DESC1.get(tanPlayer));

      GuiItem townButton =
          ItemBuilder.from(territoryIcon)
              .asGuiItem(
                  event -> {
                    event.setCancelled(true);
          ...
          ItemBuilder.from(switchPlayerIcon)
              .asGuiItem(
                  event -> {
                    ...
                    openConfirmMenu(
                        player,
                        Lang.GUI_CONFIRM_CHANGE_LEADER.get(
                            tanPlayer, iteratetanPlayer.getNameStored()),
                        confirm -> {
  ...
  public static void dispatchLandmarkGui(Player player, Landmark landmark) {
    ...
    TanChatUtils.message(
        player, Lang.LANDMARK_ALREADY_CLAIMED.get(tanPlayer, owner.getName()), MINOR_BAD);
  }
  ...
  private static void openPlayerOwnLandmark(Player player, Landmark landmark) {
    ...
    ITanPlayer tanPlayer = PlayerDataStorage.getInstance().getSync(player);
    Gui gui =
        GuiUtil.createChestGui(Lang.HEADER_LANDMARK_CLAIMED.get(tanPlayer, townData.getName()), 3);
    gui.setDefaultClickAction(event -> event.setCancelled(true));

    int quantity = landmark.computeStoredReward(townData);

    ItemStack removeTown =
        HeadUtils.makeSkullB64(
    ...
    if (quantity == 0)
      bagTexture =
          "eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNjRjMTY0YmFjMjE4NGE3NmExZWU5NjkxMzI0MmUzMzVmMWQ0MTFjYWZmNTEyMDVlYTM5YjIwNWU2ZjhmMDU4YSJ9fX0=";
    else
      bagTexture =
          "eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvYTliOTA2YjIxNTVmMTkzNzg3MDQyMzM4ZDA1Zjg0MDM5MWMwNWE2ZDNlODE2MjM5MDFiMjk2YmVlM2ZmZGQyIn19fQ==";

    ItemStack collectRessources =
        HeadUtils.makeSkullB64(
    ...
    GuiItem removeTownButton =
        ItemBuilder.from(removeTown)
            .asGuiItem(
                event -> {
                  event.setCancelled(true);
    ...
    GuiItem collectRessourcesButton =
        ItemBuilder.from(collectRessources)
            .asGuiItem(
                event -> {
                  event.setCancelled(true);
                  ...
                  TanChatUtils.message(
                      player,
                      Lang.GUI_LANDMARK_REWARD_COLLECTED.get(tanPlayer, Integer.toString(quantity)),
                      GOOD);
                  dispatchLandmarkGui(player, landmark);
                });
  ...
  public static void openConfirmMenu(
      Player player,
      String confirmLore,
      Consumer<Void> confirmAction,
      Consumer<Void> returnAction) {
    ITanPlayer tanPlayer = PlayerDataStorage.getInstance().getSync(player);
    Gui gui = GuiUtil.createChestGui(Lang.HEADER_CONFIRMATION.get(tanPlayer), 3);

    ItemStack confirm =
        HeadUtils.makeSkullB64(
            Lang.GENERIC_CONFIRM_ACTION.get(tanPlayer),
    ...
    ItemStack cancel =
        HeadUtils.makeSkullB64(
            Lang.GENERIC_CANCEL_ACTION.get(tanPlayer),
  ...
  public static void openHierarchyMenu(Player player, TerritoryData territoryData) {
    ...
    if (territoryData.canHaveOverlord()) {
      ...
      if (overlordOptional.isPresent()) {
        ...
        overlordButton =
            ItemBuilder.from(declareIndependence)
                .asGuiItem(
                    event -> {
                      event.setCancelled(true);
                      if (!territoryData.haveOverlord()) {
                        ...
                        openHierarchyMenu(
                            player,
                            territoryData); // This should trigger only if town have been kicked
                        // from region during the menu
                        return;
                      }
                      ...
                      if (territoryData.isCapital()) {
                        TanChatUtils.message(
                            player,
                            Lang.CANNOT_DECLARE_INDEPENDENCE_BECAUSE_CAPITAL.get(
                                tanPlayer, territoryData.getBaseColoredName()));
                        return;
                      }

                      openConfirmMenu(
                          player,
                          Lang.GUI_CONFIRM_DECLARE_INDEPENDENCE.get(
                              tanPlayer,
                              territoryData.getBaseColoredName(),
                              overlord.getBaseColoredName()),
                          confirm -> {
                            territoryData.removeOverlord();
                            territoryData.broadcastMessageWithSound(
                                Lang.TOWN_BROADCAST_TOWN_LEFT_REGION.get(
                                    territoryData.getName(), overlord.getName()),
                                BAD);
                            overlord.broadCastMessage(
                                Lang.REGION_BROADCAST_TOWN_LEFT_REGION.get(
        ...
        GuiItem donateToOverlordButton =
            ItemBuilder.from(donateToOverlord)
                .asGuiItem(
                    event -> {
                      event.setCancelled(true);
                      TanChatUtils.message(
        ...
        ItemStack noCurrentOverlord =
            HeadUtils.createCustomItemStack(
                Material.GOLDEN_HELMET,
                Lang.OVERLORD_GUI.get(tanPlayer),
                Lang.NO_OVERLORD.get(tanPlayer));
        overlordInfo =
            ItemBuilder.from(noCurrentOverlord).asGuiItem(event -> event.setCancelled(true));

        ItemStack joinOverlord =
            HeadUtils.createCustomItemStack(
                Material.WRITABLE_BOOK,
                Lang.BROWSE_OVERLORD_INVITATION.get(tanPlayer),
                Lang.BROWSE_OVERLORD_INVITATION_DESC1.get(
                    tanPlayer,
                    Integer.toString(territoryData.getNumberOfVassalisationProposals())));

        overlordButton =
            ItemBuilder.from(joinOverlord)
                .asGuiItem(
                    event -> {
                      event.setCancelled(true);
    ...
    if (territoryData.canHaveVassals()) {
      ItemStack vassalIcon =
          HeadUtils.createCustomItemStack(
              Material.GOLDEN_SWORD,
              Lang.VASSAL_GUI.get(tanPlayer),
              Lang.VASSAL_GUI_DESC1.get(
                  tanPlayer,
                  territoryData.getBaseColoredName(),
                  Integer.toString(territoryData.getVassalCount())));

      ItemStack vassals =
          HeadUtils.makeSkullB64(
              Lang.GUI_REGION_TOWN_LIST.get(tanPlayer),
      ...
      GuiItem vassalsButton =
          ItemBuilder.from(vassals)
              .asGuiItem(
                  event -> {
                    event.setCancelled(true);
