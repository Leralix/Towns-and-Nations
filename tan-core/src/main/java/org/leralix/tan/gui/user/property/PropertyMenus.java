...
public abstract class PropertyMenus extends BasicGui {
        ...
        .setAction(
            action -> {
              ...
              PlayerChatListenerStorage.register(
                  player, new ChangePropertyName(propertyData, p -> open()));
            })
        .asGuiItem(player, langType);
  }
    ...
    return iconManager
        ...
        .setAction(
            action -> {
              TanChatUtils.message(player, Lang.ENTER_NEW_VALUE.get(langType));
              PlayerChatListenerStorage.register(
                  player, new ChangePropertyDescription(propertyData, p -> open()));
            })
        .asGuiItem(player, langType);
  }
  ...
  protected GuiItem forSaleButton() {
    ...
    return iconManager
        ...
        .setDescription(
            Lang.GUI_BUYING_PRICE.get(
                Double.toString(total), Double.toString(price), Double.toString(taxPrice)),
            Lang.GUI_TOWN_RATE.get(
                String.format("%.2f", propertyData.getTown().getTaxOnBuyingProperty() * 100)))
        .setClickToAcceptMessage(
            Lang.GUI_LEFT_CLICK_TO_SWITCH_SALE, Lang.GUI_RIGHT_CLICK_TO_CHANGE_PRICE)
        .setAction(
            event -> {
              if (event.getClick() == ClickType.RIGHT) {
                TanChatUtils.message(player, Lang.ENTER_NEW_VALUE.get(langType));
                PlayerChatListenerStorage.register(
                    player, new ChangePropertySalePrice(propertyData, p -> open()));
    ...
    return iconManager
        ...
        .setDescription(
            Lang.GUI_BUYING_PRICE.get(
                Double.toString(total), Double.toString(price), Double.toString(taxPrice)),
            Lang.GUI_TOWN_RATE.get(
                String.format("%.2f", propertyData.getTown().getTaxOnRentingProperty() * 100)))
        .setClickToAcceptMessage(
            Lang.GUI_LEFT_CLICK_TO_SWITCH_SALE, Lang.GUI_RIGHT_CLICK_TO_CHANGE_PRICE)
        .setAction(
            event -> {
              if (event.getClick() == ClickType.RIGHT) {
                TanChatUtils.message(player, Lang.ENTER_NEW_VALUE.get(langType));
                PlayerChatListenerStorage.register(
                    player, new ChangePropertyRentPrice(propertyData, p -> open()));
              } else if (event.getClick() == ClickType.LEFT) {
                if (propertyData.isRented()) {
                  TanChatUtils.message(player, Lang.PROPERTY_ALREADY_RENTED.get(langType));
                  return;
  ...
  protected GuiItem getKickRenterButton() {
    return iconManager
        ...
        .setName(
            Lang.GUI_PROPERTY_RENTED_BY.get(langType, propertyData.getRenter().getNameStored()))
