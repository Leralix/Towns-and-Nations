...
public class RankManagerMenu extends BasicGui {
            ...
            event -> {
              if (!isEmpty) {
                TanChatUtils.message(
                    player,
                    Lang.GUI_TOWN_MEMBERS_ROLE_DELETE_ERROR_NOT_EMPTY.get(tanPlayer),
                    NOT_ALLOWED);
                return;
              }
              if (isDefaultRank) {
                TanChatUtils.message(
                    player,
                    Lang.GUI_TOWN_MEMBERS_ROLE_DELETE_ERROR_DEFAULT.get(tanPlayer),
                    NOT_ALLOWED);
                return;
              }
  ...
  private GuiItem getDefaultRankButton() {
    ...
    if (isDefaultRank) description.add(Lang.GUI_TOWN_MEMBERS_ROLE_SET_DEFAULT2.get());

    return iconManager
        .get(IconKey.SET_DEFAULT_ROLE_ICON)
        .setName(
            isDefaultRank
                ? Lang.GUI_TOWN_MEMBERS_ROLE_SET_DEFAULT_IS_DEFAULT.get(tanPlayer)
                : Lang.GUI_TOWN_MEMBERS_ROLE_SET_DEFAULT_IS_NOT_DEFAULT.get(tanPlayer))
        ...
        .setAction(
            event -> {
              if (isDefaultRank) {
                TanChatUtils.message(
                    player,
                    Lang.GUI_TOWN_MEMBERS_ROLE_SET_DEFAULT_ALREADY_DEFAULT.get(tanPlayer),
                    NOT_ALLOWED);
              } else {
  ...
  private GuiItem increaseSalaryButton() {
    return iconManager
        .get(IconKey.INCREASE_SALARY_ICON)
        .setName(Lang.GUI_LOWER_SALARY.get(tanPlayer))
        .setDescription(Lang.GUI_INCREASE_1_DESC.get(), Lang.GUI_INCREASE_10_DESC.get())
        .setAction(
            event -> {
  ...
  private GuiItem getSalaryIcon() {
    return iconManager
        .get(IconKey.CURRENT_SALARY_ICON)
        .setName(Lang.GUI_TOWN_MEMBERS_ROLE_SALARY.get(tanPlayer))
        .setDescription(
            Lang.GUI_TOWN_MEMBERS_ROLE_SALARY_DESC1.get(Integer.toString(rankData.getSalary())))
        .asGuiItem(player, langType);
  }

  private GuiItem lowerSalaryButton() {
    return iconManager
        .get(IconKey.DECREASE_SALARY_ICON)
        .setName(Lang.GUI_LOWER_SALARY.get(tanPlayer))
        .setDescription(Lang.GUI_DECREASE_1_DESC.get(), Lang.GUI_DECREASE_10_DESC.get())
        .setAction(
            event -> {
              ...
              if (currentSalary <= 0) {
                TanChatUtils.message(
                    player,
                    Lang.GUI_TOWN_MEMBERS_ROLE_SALARY_ERROR_LOWER.get(tanPlayer),
                    NOT_ALLOWED);
                return;
              }
  ...
  private GuiItem getRankLevel() {
    ...
    return iconManager
        .get(rankData.getRankEnum().getBasicRankIcon())
        .setName(
            rankEnum.getColor()
                + Lang.GUI_TOWN_MEMBERS_ROLE_PRIORITY_X.get(
                    tanPlayer, Integer.toString(rankEnum.getLevel())))
        .setDescription(
        ...
        .setAction(
            event -> {
              ...
              boolean isInferiorOrEquals =
                  playerRank.getRankEnum().getLevel() <= (rankData.getRankEnum().getLevel() + 1);

              if (isInferiorOrEquals && !isLeader) {
                TanChatUtils.message(
                    player,
                    Lang.GUI_TOWN_MEMBERS_ROLE_RANK_UP_INFERIOR_RANK.get(
                        tanPlayer, playerRank.getColoredName()));
                return;
              }
        ...
        .setAction(
            event -> {
              ...
              if (itemMaterial.getType() == Material.AIR) {
                TanChatUtils.message(
                    player, Lang.GUI_TOWN_MEMBERS_ROLE_NO_ITEM_SHOWED.get(tanPlayer));
                return;
              }
              rankData.setRankIcon(itemMaterial);
              open();
              TanChatUtils.message(
                  player, Lang.GUI_TOWN_MEMBERS_ROLE_CHANGED_ICON_SUCCESS.get(tanPlayer));
            })
        .asGuiItem(player, langType);
  }
  ...
  private GuiItem getAddPlayerButton() {
    ...
    for (ITanPlayer tanPlayer : rankData.getPlayers()) {
      description.add(
          Lang.GUI_TOWN_MEMBERS_ROLE_MEMBER_LIST_INFO_DESC.get(tanPlayer.getNameStored()));
    }
