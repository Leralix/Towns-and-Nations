...
public class RankPermissionRequirement extends IndividualRequirement {
  ...
  public RankPermissionRequirement(
      TerritoryData territoryData, ITanPlayer tanPlayer, RolePermission requiredPermission) {
  ...
  public String getLine(LangType langType) {
    if (isInvalid()) {
      return Lang.REQUIREMENT_RANK_PERMISSION_NEGATIVE.get(
          langType, requiredPermission.getName(langType));
    } else {
      return Lang.REQUIREMENT_RANK_PERMISSION_POSITIVE.get(
          langType, requiredPermission.getName(langType));
    }
  }
