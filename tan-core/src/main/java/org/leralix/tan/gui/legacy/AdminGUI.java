...
public class AdminGUI {
  ...
  public static void openLandmarks(Player player, int page) {
                    ...
                    if (!event.isShiftClick()) {
                      ...
                      org.leralix.tan.utils.FoliaScheduler.runTaskLater(
                          ...
                          () -> {
                            ...
                            player.teleportAsync(landmark.getLocation());
                          },
                          1L);
    ...
    ItemStack createLandmark =
        HeadUtils.makeSkullB64(
            Lang.ADMIN_GUI_CREATE_LANDMARK.get(langType),
    ...
    GuiItem createLandmarkGui =
        ItemBuilder.from(createLandmark)
            .asGuiItem(
                event -> {
                  event.setCancelled(true);
                  ...
                  ClaimedChunk2 claimedChunk =
                      NewClaimedChunkStorage.getInstance()
                          .get(player.getLocation().getBlock().getChunk());
  ...
  private static void openSpecificLandmarkMenu(Player player, Landmark landmark) {
    ...
    Gui gui =
        GuiUtil.createChestGui(
            Lang.HEADER_ADMIN_SPECIFIC_LANDMARK_MENU.get(langType, landmark.getName()), 3);

    ItemStack changeLandmarkName =
        HeadUtils.createCustomItemStack(
            Material.NAME_TAG,
            Lang.ADMIN_GUI_CHANGE_LANDMARK_NAME.get(langType),
            Lang.ADMIN_GUI_CHANGE_LANDMARK_NAME_DESC1.get(langType));

    ItemStack deleteLandmark =
        HeadUtils.createCustomItemStack(
            Material.BARRIER,
    ...
    ItemStack setReward =
        HeadUtils.createCustomItemStack(
            landmark.getResources(),
            Lang.SPECIFIC_LANDMARK_ICON_DESC1.get(
                langType,
                Integer.toString(ressources.getAmount()),
                ressources.getType().name().toLowerCase()),
            Lang.SPECIFIC_LANDMARK_ICON_SWITCH_REWARD.get(langType));

    GuiItem changeLandmarkNameGui =
        ItemBuilder.from(changeLandmarkName)
            .asGuiItem(
                event -> {
                  event.setCancelled(true);
                  ...
                  PlayerChatListenerStorage.register(
                      player,
                      new ChangeLandmarkName(
                          landmark,
                          Constants.getLandmarkMaxNameSize(),
                          p -> openSpecificLandmarkMenu(player, landmark)));
                });

    GuiItem deleteLandmarkGui =
        ItemBuilder.from(deleteLandmark)
            .asGuiItem(
                event -> {
                  event.setCancelled(true);
    ...
    GuiItem setRewardGui =
        ItemBuilder.from(setReward)
            .asGuiItem(
                event -> {
                  event.setCancelled(true);
                  ...
                  TanChatUtils.message(
                      player,
                      Lang.ADMIN_GUI_LANDMARK_REWARD_SET.get(
                          player,
                          Integer.toString(itemOnCursor.getAmount()),
                          itemOnCursor.getType().name()),
                      GOOD);
  ...
  private static void openSpecificRegionMenu(Player player, RegionData regionData) {
    ...
    ItemStack changeTownLeader =
        HeadUtils.createCustomItemStack(
            Material.PLAYER_HEAD,
            Lang.ADMIN_GUI_CHANGE_REGION_LEADER.get(langType),
            Lang.ADMIN_GUI_CHANGE_REGION_LEADER_DESC1.get(
                langType,
                regionData.getLeaderData().getNameStored(),
                regionData.getCapital().getName()));
    GuiItem changeTownLeaderGui =
        ItemBuilder.from(changeTownLeader)
            .asGuiItem(
                event -> {
                  event.setCancelled(true);
  ...
  private static void addCommonTerritoryDebugOption(
      Gui gui, Player player, TerritoryData territoryData) {
    LangType langType = PlayerDataStorage.getInstance().getSync(player).getLang();

    ItemStack changeRegionName =
        HeadUtils.createCustomItemStack(
            Material.NAME_TAG,
            Lang.ADMIN_GUI_CHANGE_TOWN_NAME.get(langType),
            Lang.ADMIN_GUI_CHANGE_TOWN_NAME_DESC1.get(langType, territoryData.getName()));
    ItemStack changeRegionDescription =
        HeadUtils.createCustomItemStack(
            Material.WRITABLE_BOOK,
            Lang.ADMIN_GUI_CHANGE_TOWN_DESCRIPTION.get(langType),
            Lang.ADMIN_GUI_CHANGE_TOWN_DESCRIPTION_DESC1.get(
                langType, territoryData.getDescription()));
    ItemStack deleteRegion =
        HeadUtils.createCustomItemStack(
            Material.BARRIER,
            Lang.ADMIN_GUI_DELETE_TOWN.get(langType),
            Lang.ADMIN_GUI_DELETE_TOWN_DESC1.get(langType, territoryData.getName()));

    GuiItem changeRegionNameGui =
        ItemBuilder.from(changeRegionName)
            .asGuiItem(
                event -> {
                  event.setCancelled(true);
                  TanChatUtils.message(player, Lang.ENTER_NEW_VALUE.get(player));
                  PlayerChatListenerStorage.register(
                      player,
                      new ChangeTerritoryName(
                          territoryData, 0, p -> openSpecificTerritoryMenu(player, territoryData)));
                });
    GuiItem changeRegionDescriptionGui =
        ItemBuilder.from(changeRegionDescription)
            .asGuiItem(
                event -> {
                  event.setCancelled(true);
                  TanChatUtils.message(player, Lang.ENTER_NEW_VALUE.get(player));
                  PlayerChatListenerStorage.register(
                      player,
                      new ChangeTerritoryDescription(
                          territoryData, p -> openSpecificTerritoryMenu(player, territoryData)));
                });
    GuiItem deleteRegionGui =
        ItemBuilder.from(deleteRegion)
            .asGuiItem(
                event -> {
                  event.setCancelled(true);

                  FileUtil.addLineToHistory(
                      Lang.REGION_DELETED_NEWSLETTER.get(
                          player.getName(), territoryData.getName()));
                            ...
                            overlord ->
                                TanChatUtils.message(
                                    ...
                                    Lang.CANNOT_DELETE_TERRITORY_IF_CAPITAL.get(
                                        langType, overlord.getBaseColoredName())));
                    return;
                  }
  ...
  private static void openRegionDebugChangeOwnershipPlayerSelect(
      Player player, RegionData regionData, int page) {
    LangType langType = PlayerDataStorage.getInstance().getSync(player).getLang();

    Gui gui =
        GuiUtil.createChestGui(
            Lang.HEADER_ADMIN_CHANGE_REGION_LEADER.get(langType, regionData.getName()), 6);
    ...
    for (String playerID : regionData.getPlayerIDList()) {
      ...
      ItemStack switchPlayerIcon =
          HeadUtils.getPlayerHead(Bukkit.getOfflinePlayer(UUID.fromString(playerID)));

      GuiItem switchPlayerGui =
          ItemBuilder.from(switchPlayerIcon)
              .asGuiItem(
                  event -> {
                    event.setCancelled(true);
                    FileUtil.addLineToHistory(
                        Lang.HISTORY_REGION_CAPITAL_CHANGED.get(
                            player.getName(),
                            regionData.getCapital().getName(),
                    ...
                    regionData.broadcastMessageWithSound(
                        Lang.GUI_REGION_SETTINGS_REGION_CHANGE_LEADER_BROADCAST.get(
                            iterateTanPlayer.getNameStored()),
                        GOOD);

                    if (!regionData
                        .getCapital()
                        .getID()
                        .equals(iterateTanPlayer.getTownSync().getID())) {
                      regionData.broadCastMessage(
                          Lang.GUI_REGION_SETTINGS_REGION_CHANGE_CAPITAL_BROADCAST.get(
  ...
  public static void openAdminBrowseTown(Player player, int page) {
    ...
    for (TownData townData : TownDataStorage.getInstance().getAllSync().values()) {
      ...
      HeadUtils.addLore(townIcon, "", Lang.ADMIN_GUI_LEFT_CLICK_TO_MANAGE_TOWN.get(langType));

      GuiItem townIterationGui =
    ...
    ItemStack createTown =
        HeadUtils.makeSkullB64(
            Lang.ADMIN_GUI_CREATE_TOWN.get(langType),
            "eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNWZmMzE0MzFkNjQ1ODdmZjZlZjk4YzA2NzU4MTA2ODFmOGMxM2JmOTZmNTFkOWNiMDdlZDc4NTJiMmZmZDEifX19",
            Lang.ADMIN_GUI_CREATE_TOWN_DESC1.get(langType));

    GuiItem createTownGui =
        ItemBuilder.from(createTown)
            .asGuiItem(
                event -> {
                  event.setCancelled(true);
                  TanChatUtils.message(player, Lang.ENTER_NEW_VALUE.get(player));
                  PlayerChatListenerStorage.register(
                      player, new CreateEmptyTown(p -> openAdminBrowseTown(player, 0)));
                });
  ...
  public static void openSpecificTownMenu(Player player, @NotNull TownData townData) {
    ...
    Gui gui =
        GuiUtil.createChestGui(
            Lang.HEADER_ADMIN_SPECIFIC_TOWN_MENU.get(langType, townData.getName()), 3);
    ...
    if (townData.haveOverlord()) {
      ...
      else HeadUtils.addLore(setRegionIcon, Lang.GUI_RIGHT_CLICK_TO_QUIT.get(langType));
    } else {
    ...
    GuiItem setRegionButton =
        ItemBuilder.from(setRegionIcon)
            .asGuiItem(
                event -> {
                  event.setCancelled(true);
                  if (townData.haveOverlord()) {
  ...
  private static void openChooseNewOverlord(Player player, TerritoryData territoryData, int page) {
    ...
    Gui gui =
        GuiUtil.createChestGui(
            Lang.HEADER_ADMIN_CHANGE_OVERLORD.get(langType, territoryData.getName()), 6);

    Collection<RegionData> territoryDataList =
        RegionDataStorage.getInstance().getAllSync().values();
    ITanPlayer tanPlayer = PlayerDataStorage.getInstance().getSync(player);
    List<GuiItem> guiItems = new ArrayList<>();

    for (TerritoryData potentialOverlord : territoryDataList) {
      ItemStack potentialOverlordIcon =
          potentialOverlord.getIconWithInformations(tanPlayer.getLang());
      HeadUtils.addLore(potentialOverlordIcon, Lang.GUI_GENERIC_CLICK_TO_SELECT.get(langType));

      guiItems.add(
          ItemBuilder.from(potentialOverlordIcon)
              .asGuiItem(
                  event -> {
                    event.setCancelled(true);
  ...
  private static void openTownDebugChangeOwnershipPlayerSelect(
      Player player, TownData townData, int page) {
    LangType langType = PlayerDataStorage.getInstance().getSync(player).getLang();

    Gui gui =
        GuiUtil.createChestGui(
            Lang.HEADER_ADMIN_CHANGE_TOWN_LEADER.get(langType, townData.getName()), 3);
      ...
      ItemStack playerHead =
          HeadUtils.getPlayerHead(
              ...
              Lang.GUI_TOWN_SETTINGS_TRANSFER_OWNERSHIP_TO_SPECIFIC_PLAYER_DESC1.get(
                  langType, townPlayer.getName()),
              Lang.GUI_TOWN_SETTINGS_TRANSFER_OWNERSHIP_TO_SPECIFIC_PLAYER_DESC2.get(langType));

      GuiItem playerHeadGui =
          ItemBuilder.from(playerHead)
              .asGuiItem(
                  event -> {
                    event.setCancelled(true);
                    FileUtil.addLineToHistory(
                        Lang.HISTORY_TOWN_LEADER_CHANGED.get(
                            player.getName(),
                            townData.getLeaderData().getNameStored(),
                            townPlayer.getName()));
  ...
  private static void openSpecificPlayerMenu(Player player, ITanPlayer tanPlayer) {
    ...
    ItemStack playerHead =
        HeadUtils.getPlayerHeadInformation(
            Bukkit.getOfflinePlayer(UUID.fromString(tanPlayer.getID())));

    if (tanPlayer.hasTown()) {
      ItemStack removePlayerTown =
          HeadUtils.createCustomItemStack(
      ...
      GuiItem removePlayerTownGui =
          ItemBuilder.from(removePlayerTown)
              .asGuiItem(
                  event -> {
                    event.setCancelled(true);
                    ...
                    TanChatUtils.message(
                        player,
                        Lang.ADMIN_GUI_TOWN_PLAYER_LEAVE_TOWN_SUCCESS.get(
                            langType, tanPlayer.getNameStored(), townData.getName()));
                    openSpecificPlayerMenu(player, tanPlayer);
                  });
      gui.setItem(2, 2, removePlayerTownGui);
    } else {
      ItemStack addPlayerTown =
          HeadUtils.createCustomItemStack(
              Material.SPRUCE_DOOR, "Add player to town", "Add player to town");

      GuiItem addPlayerTownGui =
          ItemBuilder.from(addPlayerTown)
              .asGuiItem(
                  event -> {
                    event.setCancelled(true);
