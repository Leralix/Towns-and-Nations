...
public class OtherUpgradeRequirement extends IndividualRequirement {
  ...
  public OtherUpgradeRequirement(
      Upgrade otherUpgrade, int requiredLevel, TerritoryData territoryData) {
  ...
  public String getLine(LangType langType) {
    ...
    if (isInvalid()) {
      return Lang.REQUIREMENT_UPGRADE_LEVEL_NEGATIVE.get(
          langType,
          otherUpgrade.getName(langType),
          String.valueOf(currentLevel),
          String.valueOf(requiredLevel));
    } else {
      return Lang.REQUIREMENT_UPGRADE_LEVEL_POSITIVE.get(
          langType,
          otherUpgrade.getName(langType),
          String.valueOf(currentLevel),
          String.valueOf(requiredLevel));
