            ...
            iconManager
                ...
                .setAction(
                    event -> {
                      ...
                      if (!territoryData.getRank(player).hasPermission(permission)
                          && !territoryData.isLeader(player)) {
                        TanChatUtils.message(
                            player,
                            Lang.ERROR_CANNOT_CHANGE_PERMISSION_IF_PLAYER_RANK_DOES_NOT_HAVE_IT.get(
                                player),
