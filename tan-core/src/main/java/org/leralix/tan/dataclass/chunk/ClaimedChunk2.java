...
public abstract class ClaimedChunk2 {
  ...
  public Vector2D getMiddleVector2D() {
    ...
    return new Vector2D(
        vector.getX() * 16 + 8, vector.getZ() * 16 + 8, vector.getWorldID().toString());
  }
  ...
  protected abstract boolean canPlayerDoInternal(
  ...
  public boolean canTerritoryClaim(Player player, TerritoryData territoryData) {
    ...
    TanChatUtils.message(
        player, Lang.CHUNK_ALREADY_CLAIMED_WARNING.get(player, getOwner().getBaseColoredName()));
    return false;
  }
  ...
  public boolean containsPosition(Vector3D position) {
    ...
    return chunk.getX() == chunkToCompare.getX()
        && chunk.getZ() == chunkToCompare.getZ()
        && chunk.getWorld() == chunkToCompare.getWorld();
  }
}
