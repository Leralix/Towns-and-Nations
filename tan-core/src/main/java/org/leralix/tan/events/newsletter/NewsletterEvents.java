...
public class NewsletterEvents implements TanListener {
  ...
  public void onAttackDeclared(AttackDeclaredEvent event) {
    newsletterStorage.register(
        new AttackDeclaredNewsletter(event.getDefenderTerritory(), event.getAttackerTerritory()));
  }

  @EventHandler
  public void onAttackWonByAttacker(AttackWonByAttackerEvent event) {
    newsletterStorage.register(
        new AttackWonByAttackerNewsletter(
            event.getAttackerTerritory(), event.getDefenderTerritory()));
  }

  @EventHandler
  public void onAttackWonByDefender(AttackWonByAttackerEvent event) {
    newsletterStorage.register(
        new AttackWonByDefenderNewsletter(
            event.getAttackerTerritory(), event.getDefenderTerritory()));
  }
  ...
  public void onDefenderAcceptDemands(AttackCancelledByDefenderEvent event) {
    newsletterStorage.register(
        new AttackCancelledByDefenderNewsletter(
            event.getDefenderTerritory(), event.getAttackerTerritory()));
  }
  ...
  public void onTerritoryIndependence(TerritoryIndependenceEvent event) {
    newsletterStorage.register(
        new TerritoryIndependentNews(event.getTerritory(), event.getFormerOverlord()));
  }

  @EventHandler
  public void onTerritoryVassalAccepted(TerritoryVassalAcceptedEvent event) {
    newsletterStorage.register(
        new TerritoryVassalAcceptedNews(event.getNewOverlord(), event.getTerritory()));
  }

  @EventHandler
  public void onTerritoryVassalForced(TerritoryVassalForcedEvent event) {
    newsletterStorage.register(
        new TerritoryVassalForcedNews(event.getNewOverlord(), event.getTerritory()));
  }

  public void onTerritoryVassalProposal(TerritoryVassalProposalEvent event) {
    newsletterStorage.register(
        new TerritoryVassalProposalNews(event.getPotentialOverlord(), event.getTerritory()));
  }
  ...
  public void onLandmarkClaimed(LandmarkClaimedEvent event) {
    newsletterStorage.register(
        new LandmarkClaimedNewsletter(event.getLandmark(), event.getNewOwner()));
  }

  @EventHandler
  public void onLandmarkUnclaimed(LandmarkUnclaimedEvent event) {
    newsletterStorage.register(
        new LandmarkUnclaimedNewsletter(event.getLandmark(), event.getOldOwner()));
  }
}
