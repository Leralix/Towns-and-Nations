...
public class TerritoryVassalAcceptedDAO extends NewsletterSubDAO<TerritoryVassalAcceptedNews> {
  ...
  public void save(TerritoryVassalAcceptedNews newsletter) {
    String sql =
        "INSERT INTO "
            + TABLE_NAME
            + " (id, proposingTerritoryID, receivingTerritoryID) VALUES (?, ?, ?)";
    try (Connection conn = dataSource.getConnection();
        PreparedStatement ps = conn.prepareStatement(sql)) {
  ...
  public TerritoryVassalAcceptedNews load(UUID id, long date) {
    String sql =
        "SELECT proposingTerritoryID, receivingTerritoryID FROM " + TABLE_NAME + " WHERE id = ?";
    try (Connection conn = dataSource.getConnection();
        PreparedStatement ps = conn.prepareStatement(sql)) {
      ...
      try (ResultSet rs = ps.executeQuery()) {
        if (rs.next()) {
          ...
          return new TerritoryVassalAcceptedNews(
              id, date, proposingTerritoryID, receivingTerritoryID);
        }
      }
    } catch (SQLException e) {
